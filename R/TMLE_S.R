...
estimateTMLEprob <- function(treatment, eventObserved, time, survHaz, cenHaz, treatProb, tau){
  ...
  ## parameter
  maxTime <- dim(survHaz)[1]/length(unique(survHaz$ID))
  ...
  for (TimePoint in tau){
    ...
    while((!converged) && iter <= 20){
      ...
      H1 <- - (ind * rep(SurvProb1[which(dlong$t == TimePoint)], each=max(dlong$t))) / bound(SurvProb1 * treatProb[SurvHaz$ID] * CenProb1)
      H0 <- - (ind * rep(SurvProb0[which(dlong$t == TimePoint)], each=max(dlong$t))) / bound(SurvProb0 * (1-treatProb[SurvHaz$ID]) * CenProb0)
