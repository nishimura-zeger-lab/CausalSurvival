...
estimateAdjIPWrmst <- function(eventTime, censorTime, treatment, covariates, covariates.names,
                              J=5, gR.estimate="glm",
  ...
  ## Estimator
  adipw  <- tau + c(mean(DT0ipw), mean(DT1ipw))
  ...
  ## result
  out <- list(rmst1=adipw[2], rmst0=adipw[1], std.error.diff=sdn)
  return(out)
}
